...
ggplot(d, aes(準誤用の平均出現割合, 準正用の平均出現割合)) + geom_text(aes(label=orth.base, color=判定, size=全コーパスにおける出現割合の平均)) + ggtitle('準誤用対準誤用の出現割合の平均') + scale_size_continuous(range = c(1, 12))
ggplot(d, aes(準誤用の平均出現割合, 準正用の平均出現割合)) + geom_text(aes(label=orth.base, color=判定, size=全コーパスにおける出現割合の平均)) + ggtitle('準誤用対準誤用の出現割合の平均（拡大）') + scale_size_continuous(range = c(1.5, 18)) + scale_x_continuous(limits=c(0, 200)) + scale_y_continuous(limits=c(0, 100))
dev.off()
