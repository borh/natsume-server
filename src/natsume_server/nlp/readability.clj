(ns natsume-server.nlp.readability
  (:require [clojure.java.shell :as shell]
            [clojure.string :as string]
    #_[natsume-server.component.database :as db]
...
(defn chunk-depth
  ...
  (loop [chunks t
    ...
    (let [last-depth (peek depth-stack)
          chunk (first chunks)]
      (if (= -1 (:chunk/link chunk))
        (apply + depth-stack)
        #_(/ (reduce + depth-stack) (count depth-stack))
        (recur (next chunks)
               (if (> (- (:chunk/link chunk) (:chunk/id chunk)) 1)
...
(defn link-distance
  ...
  (if (< (count t) 2)
    ...
    (let [chunks (drop-last t)]                             ; we drop last chunk as it has no link
      (apply + (for [chunk chunks] (- (:chunk/link chunk) (:chunk/id chunk))))
      #_(/
          (reduce +
                  (for [chunk chunks] (- (:chunk/link chunk) (:id chunk))))
          (count chunks)))))
...
(defn BCCWJ-word-log-freq
  ...
  (let [tokens (am/tree-to-morphemes-flat t)]
    (if (zero? (count tokens))
      0.0
      (->> tokens
           (map #(select-keys % [:morpheme/pos-1 :morpheme/lemma]))
           (map #(get-in BCCWJ-word-map [(:morpheme/pos-1 %) (:morpheme/lemma %)] 0)) ; value for missing calculated using Good-Turing
           (map (partial + 1))
           (map #(Math/log10 %))
           (apply +)))))
...
(defn JLPT-word-level
  ...
  (let [content-tokens (filter
                         (fn [x] (re-seq #"(名詞|形容詞|動詞|副詞|形状詞|接頭辞|助動詞|接尾辞)" (:morpheme/pos-1 x)))
    ...
    (float
      (if (zero? content-tokens-count)
        ...
        (/
          (->> content-tokens
               (map #(select-keys % [:morpheme/orth-base :morpheme/lemma]))
               (map vals)
...
(defn predicate-count
  ...
  (reduce
    (fn [predicates chunk]
      (if (some #(= (:morpheme/c-form %) "終止形-一般") (:chunk/tokens chunk))
        (inc predicates)
        predicates))
    0
    t))
  ...
  (reduce
    (fn [predicates chunk]
      (if (some
            (fn [token]
              (or
                (= (:morpheme/c-form token) "終止形-一般")
                (= (:morpheme/pos-1 token) "動詞")
                (and (= (:morpheme/pos-1 token) "助動詞")
                     (not= (:morpheme/orth token) "な")
                     (re-seq #"(だ|です)" (:morpheme/lemma token)))
                (= (:morpheme/pos-2 token) "終助詞")
                (= (:morpheme/pos-2 token) "句点")))
            (:chunk/tokens chunk))
        (inc predicates)
        predicates))
    0
...
(comment
  (def core-sentence-data
    {:length       (fnk [sentence :- schema/Str] (count sentence))
     :chunk/tokens (fnk [tree :- [Chunk]] (token-count tree))
  ...
  (def char-type-stats
    {:char-types (fnk [sentence :- schema/Str] (writing-system-count sentence))})
...
(defn sentence-statistics
  ...
  (merge
    {:text         s
     ...
     :unigrams     (collocations/extract-unigrams tree)
     :chunks       (chunk-count tree)
     :predicates   (predicate-count-shibasaki tree)
     :collocations (collocations/extract-collocations tree)
     :jlpt-level   (JLPT-word-level tree)
     :bccwj-level  (BCCWJ-word-log-freq tree)
     :link-dist    (link-distance tree)
     :chunk-depth  (chunk-depth tree)}
    (goshu-map tree)
    (writing-system-count s)))
...
(comment
  ...
  (defnk sentence-readability [tree :- [Chunk] text :- schema/Str]
...
(comment
  #_(def average-readability-graph
      (assoc readability-graph
        :chunk/tokens (by-sentences tokens)
